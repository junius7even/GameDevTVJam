...
{
	[SerializeField] bool drawDebugRaycasts = true;	//Should the environment checks be visualized

	[Header("Movement Properties")]
	[SerializeField] float speed = 8f;				//Player speed
	[SerializeField] float crouchSpeedDivisor = 3f;	//Speed reduction when crouching
	[SerializeField] float coyoteDuration = .05f;		//How long the player can jump after falling
	[SerializeField] float maxFallSpeed = -25f;		//Max speed player can fall

	[Header("Jump Properties")]
	[SerializeField] float jumpForce = 6.3f;			//Initial force of jump
	[SerializeField] float crouchJumpBoost = 2.5f;	//Jump boost when crouching
	[SerializeField] float hangingJumpForce = 15f;	//Force of wall hanging jumo
	[SerializeField] float jumpHoldForce = 1.9f;		//Incremental force when jump is held
	[SerializeField] float jumpHoldDuration = .1f;	//How long the jump key can be held

	[Header("Environment Check Properties")]
	[SerializeField] float footOffset = .4f;			//X Offset of feet raycast
	[SerializeField] float eyeHeight = 1.5f;			//Height of wall checks
	[SerializeField] float reachOffset = .7f;			//X offset for wall grabbing
	[SerializeField] float headClearance = .5f;		//Space needed above the player's head
	[SerializeField] float groundDistance = .2f;		//Distance player is considered to be on the ground
	[SerializeField] float grabDistance = .4f;		//The reach distance for wall grabs
	[SerializeField] LayerMask groundLayer;			//Layer of the ground

	[Header ("Status Flags")]
	[SerializeField] bool isOnGround;					//Is the player on the ground?
	[SerializeField] bool isJumping;					//Is player jumping?
	[SerializeField] bool isHanging;					//Is player hanging?
	[SerializeField] bool isCrouching;				//Is player crouching?
	[SerializeField] bool isHeadBlocked;
